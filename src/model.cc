...
real Model::binaryLogistic(int32_t target, bool label) {
  ...
  if (args.model != model_name::pvdm && args.model != model_name::pvdbow) { 
...
real Model::softmax(int32_t target) {
  ...
  /**
  * multi context model
  * hidden_ = 1/C * wi_ * (w1 + ... +  wC)
  * output_ : u
  * u = wo_j * hidden_i
  * * (matrix or vector).mul: 내부에서 원소 초기화 함.
  **/
  ...
  for (int32_t i = 0; i < osz_; i++) {
    // label = iverson bracket output.
    real label = (i == target) ? 1.0 : 0.0;
    
    // log-linear classification obtain to multinomial distribution.
    output_[i] /= z;
    
    /**
    * label = output_[i]: update equation for output weight
    * 실제 라벨과 차이가 있으면 alpha는 음의 방향으로 크다.
    * 실제 라벨과 차이가 있다면 alpha에 의해서 grad_은 자연스럽게 local minima를 향해 수렴된다. 
    * wo_도 마찬가지.
    **/
...
void Model::buildTree(const std::vector<int64_t>& counts) {
  ...
  // word들은 leaf로 취급
  int32_t leaf = osz_ - 1;
  // 노드는 osz_만큼 생성하고 osz_ 이상부터 인덱스로 사용
  int32_t node = osz_;
  // leaf가 아닌 노드들 초기화
