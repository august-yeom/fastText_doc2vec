...
void Dictionary::load(std::ifstream& ifs, std::ifstream& modelIfs) {
  words_.clear();
  for (int32_t i = 0; i < MAX_VOCAB_SIZE; i++) {
    word2int_[i] = -1;
  }
  modelIfs.read((char*) &size_, sizeof(int32_t));
  modelIfs.read((char*) &nwords_, sizeof(int32_t));
  modelIfs.read((char*) &nlabels_, sizeof(int32_t));
  modelIfs.read((char*) &ntokens_, sizeof(int64_t));
  for (int32_t i = 0; i < size_; i++) {
    char c;
    entry e;
    while ((c = modelIfs.get()) != 0) {
      e.word.push_back(c);
    }
    modelIfs.read((char*) &e.count, sizeof(int64_t));
    modelIfs.read((char*) &e.type, sizeof(entry_type));
    words_.push_back(e);
    word2int_[find(e.word)] = i;
  }
