...
namespace info {
  ...
  std::atomic<int32_t> allLabels(0);
...
void saveDocVectors(Dictionary& dict, Matrix& input) {
  std::ofstream ofs(args.output + ".vec");
  if (!ofs.is_open()) {
  ...
  for (int32_t i = 0; i < dict.nlabels(); i++) {
    std::string label = dict.getLabel(i);
    vec.zero();
    vec.addRow(input, i + nwords + args.bucket);
...
void loadModel(std::string documentsFilename, std::string modelFilename, Dictionary& dict,
                         Matrix& input, Matrix& output) {
  std::ifstream ifs(documentsFilename);
  std::ifstream modelIfs(modelFilename);
  if (!ifs.is_open()) {
    std::cerr << "Model file cannot be opened for loading!" << std::endl;
    exit(EXIT_FAILURE);
  }
  args.load(modelIfs);
  dict.load(ifs, modelIfs);
  input.load(modelIfs, dict.nlabels());
  output.load(modelIfs);
...
void printInfo(Model& model, real progress) {
  ...
  std::cout << "  allN: " << std::setprecision(6) << info::allN;
  std::cout << "  allLoss: " << std::setprecision(6) << info::allLoss;
...
void pvdbow(Dictionary& dict, Model& model,
              const std::vector<int32_t>& line,
              const std::vector<int32_t>& labels,
              double& loss, int32_t& nexamples) {
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line.size()) {
        loss += model.update(labels, line[w+c]);
        nexamples++;
      }
    }
  }
...
void pvdm(Dictionary& dict, Model& model,
          const std::vector<int32_t>& line,
          const std::vector<int32_t>& labels,
          double& loss, int32_t& nexamples) {
...
void embeddingThread(Dictionary& dict, Matrix& input, Matrix& output,
                     int32_t threadId) { 
  std::ifstream ifs(args.input);
  utils::seek(ifs, threadId * utils::size(ifs) / args.thread);
  ...
  while (info::allLabels < args.epoch * nlabels) {
    deltaCount = dict.getLine(ifs, line, labels, model.rng);
    ...
    if (args.model == model_name::pvdm) {
      pvdm(dict, model, line, labels, loss, nexamples);
    } else if (args.model == model_name::pvdbow) {
      pvdbow(dict, model, line, labels, loss, nexamples);
    }
    ...
    if (tokenCount > args.lrUpdateRate) {
      ...
      loss = 0.0;
      nexamples = 0;
      progress = real(info::allLabels) / (args.epoch * nlabels);
      model.setLearningRate(args.lr * (1.0 - progress));
      if (threadId == 0) {
        printInfo(model, progress);
      }
...
void embedding(int argc, char** argv) {
  ...
  Matrix input, output;
  time_t t0 = time(nullptr);
  loadModel(args.input, args.modelInput, dict, input, output);
  double trainTime = difftime(time(nullptr), t0);
  std::cout << "Model loading time: " << trainTime << " sec " << std::endl; 
  std::cout << "size: " << dict.nsizes() << " nlabels: " << dict.nlabels() << " nwords: " << dict.nwords() << " ntokens: " << dict.ntokens() << std::endl;
  ...
  t0 = time(nullptr);
  for (int32_t i = 0; i < args.thread; i++) {
    threads.push_back(std::thread(&embeddingThread, std::ref(dict),
                                  std::ref(input), std::ref(output), i));
  }
  for (auto it = threads.begin(); it != threads.end(); ++it) {
    it->join();
  ...
  trainTime = difftime(time(nullptr), t0);
  std::cout << "Train time: " << trainTime << " sec" << std::endl;

  if (args.output.size() != 0) {
    saveDocVectors(dict, input);
  }
